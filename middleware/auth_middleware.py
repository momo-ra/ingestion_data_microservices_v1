...
from fastapi import HTTPException, Security, WebSocket, Request, Response
...
if not JWT_SECRET or not JWT_ALGORITHM:
    logger.error("JWT_SECRET or JWT_ALGORITHM configuration is missing")
    raise ValueError("JWT configuration is missing. Please set JWT_SECRET and JWT_ALGORITHM in your configuration.")

class CustomHTTPBearer(HTTPBearer):
    """Custom HTTPBearer that returns standardized error responses"""
...
def verify_token(token: str) -> Dict[str, Any]:
    ...
    Raises HTTPException with standardized response if token is invalid.
    """
    try:
        ...
        if "user_id" not in payload:
            ...
            raise HTTPException(
                status_code=401,
                detail=fail_response(
                    message="Invalid token structure: missing user_id",
                    data={"error_type": "invalid_token_structure"}
                )
            )
